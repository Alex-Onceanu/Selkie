...
private:
  void updateParticlesPerCell()
  {
    ...
    int combien = particleCount() / 9; 
    auto miniUpdate = [&](int i) {
      for(int particle = 0; particle < combien; particle++)
      {
        auto pos = position(particle + i * combien);
        _particlesPerCell[idx1d(std::floor(pos.x), std::floor(pos.y), std::floor(pos.z))].push_back(particle + i * combien);
      }
    };
...
SphSolver gSolver(0.08, 0.5, 1e3, sk::math::vec3(0, -9.8, 0.), 0.01, 7.0);
...
void init()
{
  gSolver.initScene(16, 12, 6, 9, 8, 4);
}
